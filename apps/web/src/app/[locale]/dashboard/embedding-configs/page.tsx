...
export default function EmbeddingConfigsPage() {
  const t = useTranslations("embeddingConfigsPage");
  const tToast = useTranslations("toasts");
  const [configs, setConfigs] = useState<EmbeddingConfig[]>([]);
  const [envDefault, setEnvDefault] = useState<EmbeddingEnvDefault | null>(null);
  ...
  const showApiBase = form.provider === "openai" || form.provider === "ollama";
  ...
  const credentialsReady = useMemo(() => {
    ...
    if (form.provider === "ollama") {
      return Boolean(form.api_base.trim());
    }
  ...
  const fetchData = useCallback(async () => {
    try {
      const [configsData, providersData, envDefaultData] = await Promise.all([
        api.get("/api/embedding-configs") as Promise<EmbeddingConfig[]>,
        api.get("/api/embedding-configs/providers") as Promise<{ providers: ProviderMeta[] }>,
        api.get("/api/embedding-configs/env-default") as Promise<EmbeddingEnvDefault>,
      ]);
      ...
      if (error instanceof ApiError) {
        ...
        toast.error(tToast("embeddingConfigLoadError"));
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleFetchModels = async () => {
    ...
    try {
      const data = (await api.post("/api/embedding-configs/fetch-models", buildProbePayload())) as {
        models: FetchedModel[];
        source: string;
      };
      ...
      if (error instanceof ApiError) {
        ...
        toast.error(tToast("embeddingConfigFetchModelsError"));
      }
    } finally {
  ...
  const handleVerify = async () => {
    ...
    try {
      const data = (await api.post(
        "/api/embedding-configs/verify",
        buildProbePayload()
      )) as { success: boolean; message: string };
      if (data.success) {
        setIsVerified(true);
        toast.success(tToast("embeddingConfigVerifySuccess"));
      } else {
        setIsVerified(false);
        toast.error(data.message || tToast("embeddingConfigVerifyFailed"));
      }
    } catch (error) {
      setIsVerified(false);
      if (error instanceof ApiError) {
        ...
        toast.error(tToast("embeddingConfigVerifyFailed"));
      }
    } finally {
  ...
  const handleSubmit = async () => {
    if (!form.name.trim() || !form.model.trim()) {
      toast.error(tToast("embeddingConfigValidationError"));
      return;
    }
    if (!editingConfig && selectedProviderMeta?.requires_api_key && !form.api_key.trim()) {
      toast.error(tToast("embeddingConfigApiKeyRequired"));
      return;
    }
    if (!isVerified) {
    ...
    try {
      ...
      if (editingConfig) {
        const updated = await api.put(`/api/embedding-configs/${editingConfig.id}`, payload);
        setConfigs((prev) =>
          prev.map((item) => (item.id === editingConfig.id ? updated : item))
        );
        toast.success(tToast("embeddingConfigUpdateSuccess"));
      } else {
        const created = await api.post("/api/embedding-configs", payload);
        setConfigs((prev) => [created, ...prev]);
        toast.success(tToast("embeddingConfigCreateSuccess"));
      }
      ...
      if (error instanceof ApiError) {
        ...
        toast.error(
          editingConfig ? tToast("embeddingConfigUpdateError") : tToast("embeddingConfigCreateError")
        );
      }
    } finally {
      setIsSaving(false);
  ...
  const deleteConfig = async (id: number) => {
    try {
      await api.delete(`/api/embedding-configs/${id}`);
      setConfigs((prev) => prev.filter((item) => item.id !== id));
      toast.success(tToast("embeddingConfigDeleteSuccess"));
      await fetchData();
    } catch (error) {
      if (error instanceof ApiError) {
        toast.error(error.message);
      } else {
        toast.error(tToast("embeddingConfigDeleteError"));
      }
    }
  };

  const toggleActive = async (config: EmbeddingConfig) => {
    try {
      await api.put(`/api/embedding-configs/${config.id}`, {
        is_active: !config.is_active,
      });
      toast.success(tToast("embeddingConfigUpdateSuccess"));
      await fetchData();
      ...
      if (error instanceof ApiError) {
        ...
        toast.error(tToast("embeddingConfigUpdateError"));
      }
    }
  };

  const setAsDefault = async (config: EmbeddingConfig) => {
    if (config.is_default) return;
    if (!config.is_active) {
      toast.error(tToast("embeddingConfigSetDefaultRequiresActive"));
      return;
    }
    try {
      await api.put(`/api/embedding-configs/${config.id}`, {
      ...
      toast.success(tToast("embeddingConfigDefaultSuccess"));
      await fetchData();
    } catch (error) {
      if (error instanceof ApiError) {
        toast.error(error.message);
  ...
  const setEnvAsDefault = async () => {
    ...
    try {
      await api.post("/api/embedding-configs/env-default/set-default");
      toast.success(tToast("embeddingConfigDefaultSuccess"));
      await fetchData();
    } catch (error) {
      if (error instanceof ApiError) {
            ...
            {isLoading ? (
              ...
              <Card>
                <CardContent className="flex flex-col items-center py-16 text-center">
                  <Binary className="size-12 text-muted-foreground/50" />
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
                ...
                <div className="grid gap-4 py-2">
                  ...
                  {showApiBase ? (
                    <div className="grid gap-2">
                      <Label htmlFor="embed-api-base">{t("apiBaseLabel")}</Label>
                      <Input
                        id="embed-api-base"
                        value={form.api_base}
                  ...
                  {selectedProviderMeta?.requires_api_key ? (
                  <div className="grid gap-2">
                    <Label htmlFor="embed-api-key">{t("apiKeyLabel")}</Label>
                    <Input
                      id="llm-api-key"
                      type="password"
                      value={form.api_key}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <Label htmlFor="embed-default">{t("defaultLabel")}</Label>
                  </div>
