...
export default function UploadPage({
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      toast.success(result.message || t("uploadFileSuccess"));
    } catch (error) {
  ...
  const startProcessing = async () => {
    ...
    try {
      ...
      toast.error(t("uploadProcessStartFailed"));
    }
  };
  ...
  const checkProcessingStatus = async () => {
    ...
    try {
      ...
      const status: Record<string, TaskStatus> = await api.get(
        `/api/knowledge-base/${id}/documents/tasks?task_ids=${taskIds}`
      );
      ...
      if (allCompleted) {
        ...
        toast.success(t("uploadAllFilesProcessed"), { duration: Infinity });
      }
    } catch (error) {
    ...
    <DashboardLayout>
      <DashboardPageContainer className="space-y-8">
        ...
        <div className="flex justify-end space-x-4">
          {showSuccessModal ? (
            <button
              onClick={() => router.push(`/dashboard/knowledge/${id}`)}
              className="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2"
            >
              Done
            </button>
