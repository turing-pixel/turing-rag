...
def _normalize_openai_api_base(api_base: str) -> str:
    ...
    if base.endswith("/v1") or "/v1/" in f"{base}/":
        return base
    if "compatible-mode" in base or "openai.azure.com" in base:
        return base
    return f"{base}/v1"
...
def _is_chat_model(model_id: str) -> bool:
    ...
    if any(mid.startswith(prefix) for prefix in _EMBEDDING_PREFIXES):
        return False
    if mid.endswith("-embed") or "embedding" in mid:
        return False
    return True
...
def _catalog_fallback(provider: str) -> List[ModelOption]:
    ...
    if definition.default_model and not any(m[0] == definition.default_model for m in models):
        models.insert(0, (definition.default_model, definition.default_model))
    if provider == "ollama" and definition.default_model:
        return [ModelOption(id=definition.default_model, label=definition.default_model)]
...
def resolve_credentials(
    *,
    ...
    if config_id is not None:
        config = get_llm_config(db, config_id, user_id=user_id)
        if not config:
...
def fetch_available_models(
    *,
    ...
    if creds.provider == "ollama":
        models = _fetch_ollama_models(creds.api_base)
        if models:
            return models, "api"
        return _catalog_fallback(creds.provider), "catalog"

    try:
        models = _fetch_openai_compatible_models(creds.api_base, creds.api_key or "")
        if models:
            return models, "api"
    except Exception:
        pass
...
def _fetch_ollama_models(api_base: str) -> List[ModelOption]:
    base = (api_base or "").strip().rstrip("/")
    url = f"{base}/api/tags"
...
def _fetch_openai_compatible_models(api_base: str, api_key: str) -> List[ModelOption]:
    client = OpenAI(
        api_key=api_key,
