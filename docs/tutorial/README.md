...
可查阅： `apps/api/app/services/chat_service.py`
...
详细代码可以参考： `apps/api/app/services/document_processor.py`
...
代码可查阅： `apps/api/app/services/chat_service.py`
...
- Chat 页面：`apps/web/src/app/dashboard/chat/[id]/page.tsx`
- 引用信息展示：`apps/web/src/components/chat/answer.tsx`
...
目前已经通过 Factory 模式， 支持了不同的向量数据库、不同的大模型，例如 Ollama 也有同学在支持， 可以参考 `apps/api/app/services/vector_store/factory.py` 这个文件。
...
确保 `apps/api/requirements.txt` 文件中指定的依赖项版本是可用的。例如，将 `langchain-deepseek` 的版本更新为 `==0.1.1`：
...
在 `apps/api/app/api/api_v1/auth.py` 文件中添加错误处理，以捕获注册过程中可能出现的网络错误和无法访问的服务器问题：
...
在 `apps/api/Dockerfile` 和 `docker-compose.yml` 文件中添加重试机制，以处理构建过程中可能出现的网络错误：
